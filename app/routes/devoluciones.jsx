...
export const action = async ({ request }) => {
  ...
  if (!shop) {
    return { ok: false, error: "Falta el dominio de la tienda (shop)." };
  }
  ...
  if (intent === "lookup") {
    try {
      ...
      return { ok: true, order: normalizeOrder(match), shop };
    } catch {
  ...
  if (intent === "submit_return") {
    try {
      ...
      return { ok: true, saved: true, shop };
    } catch {
...
export default function PublicReturnsPortal() {
  const { reasons, pickupCost, prefillOrder, prefillEmail, prefillShop } = useLoaderData();
  ...
  return (
    <main style={{ maxWidth: 960, margin: "30px auto", padding: "0 16px" }}>
      <h1>Portal de devoluciones</h1>
      <section style={{ border: "1px solid #ddd", padding: 16, borderRadius: 8 }}>
        <h2 style={{ marginTop: 0 }}>1) Buscar pedido</h2>
        <Form method="post">
          <input type="hidden" name="intent" value="lookup" />
          <input type="hidden" name="shop" value={actionData?.shop || prefillShop} />
          <div style={{ display: "grid", gap: 12, maxWidth: 560 }}>
            <label>
              Numero de pedido
              <input name="orderNumber" required defaultValue={prefillOrder} />
            </label>
            <label>
              Correo del pedido
              <input name="email" type="email" required defaultValue={prefillEmail} />
            </label>
      ...
      {order ? (
        <section style={{ marginTop: 16, border: "1px solid #ddd", padding: 16, borderRadius: 8 }}>
          <h2 style={{ marginTop: 0 }}>2) Selecciona productos y envia solicitud</h2>
          <ReturnsRequestForm
            order={order}
            reasons={reasons}
            pickupCost={pickupCost}
            shop={actionData?.shop || prefillShop}
          />
          {actionData?.saved ? (
...
function ReturnsRequestForm({ order, reasons, pickupCost, shop }) {
